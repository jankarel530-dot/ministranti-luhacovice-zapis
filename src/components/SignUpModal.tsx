...
import { formatCzechDate, triggerCelebrationConfetti, isPublicSignupWindowClosed } from '../utils/helpers';
...
export const SignUpModal: React.FC<SignUpModalProps> = ({
  ...
  const isLocked = mass.isLocked || globalLockSignups || (!isAdmin && isPublicSignupWindowClosed(mass));
  const isAutoLocked = !mass.isLocked && !globalLockSignups && isLocked;
            ...
            <div className="p-3.5 rounded-xl bg-amber-50 dark:bg-amber-950/60 border border-amber-300 dark:border-amber-800 text-amber-900 dark:text-amber-200 text-xs font-bold flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[11px] opacity-90 font-medium">
                  {isAutoLocked
                    ? 'Zapisování bylo automaticky uzamčeno 45 minut po začátku mše svaté.'
                    : 'Správce nebo kněz dočasně uzamkl zapisování pro tuto mši svatou.'}
                </p>
              </div>
            </div>
          )}
