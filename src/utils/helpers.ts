...
import { LiturgicalRank, LiturgicalColor, Mass } from '../types';

const PUBLIC_SIGNUP_CUTOFF_MINUTES = 45;

export function isPublicSignupWindowClosed(mass: Mass): boolean {
  if (!mass.date || !mass.time) return false;
  const massStart = new Date(`${mass.date}T${mass.time}:00`);
  if (isNaN(massStart.getTime())) return false;
  const cutoff = new Date(massStart.getTime() + PUBLIC_SIGNUP_CUTOFF_MINUTES * 60 * 1000);
